...
1. 在「产品管理」中选择一个或多个产品，右键选择「修改库存」
2. 输入负数并确认
3. 在产品图片列表中分别选择「裸卡」「挂件袋」或「带卡砖」，并调整每个商品的减少数量
4. 选择清单图片的保存位置
5. 程序导出纵向清单图片，并按每行设置的数量减少对应产品库存

取消包装选择或取消保存时，库存不会改变。

## 订单列表

- 每次选择「确定并导出」都会创建一张货单，默认状态为「待发货」
- 货单支持「待发货」「已发货」「已成交」三种状态
- 订单列表可展开或收起商品图片、包装类型和数量
- 可编辑货单编号，增加或移除商品种类，并修改包装和数量
- 修改商品和数量时会按差额调整库存，并同步更新销量排行榜
- 可弹出查看货单详情，也可按当前货单内容重新导出库存减少清单
- 每张货单可通过红色「删除订单」按钮删除，删除后自动恢复对应库存并更新销量排行榜
- 选择「仅减少库存」不会创建货单，也不会计入销量排行榜
